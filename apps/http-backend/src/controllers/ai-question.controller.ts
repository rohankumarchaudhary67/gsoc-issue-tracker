...
const askAiQuestionWithIssue = asyncHandler(
    async (req: Request, res: Response): Promise<any> => {
        ...
        if (!question || !issueId) {
            return res
                .status(400)
                .json(new ApiResponse(400, 'Question & issueId is required'));
        }

        if (user.aiQuestionLimit <= user.currentAiQuestion) {
            return res
                .status(400)
                .json(new ApiResponse(400, 'freeTrial limit reached'));
        }
        ...
        if (!issue) {
            return res
                .status(404)
                .json(new ApiResponse(404, 'Issue not found'));
        }
        ...
        return res
            ...
            .json(
                new ApiResponse(
                    ...
                    'AI question asked successfully'
                )
            );
    }
);

export { askAiQuestionWithIssue };
