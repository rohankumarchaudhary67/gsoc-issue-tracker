...
const fetchUsage = asyncHandler(async (req: Request, res: Response):Promise<any>=>{
    ...
    try {
        ...
        if(!usage) {
            return res.status(404).json(
                new ApiError(404, "No usage found")
            );
        }
...
const upgradeUsage = asyncHandler(async (req: Request, res: Response):Promise<any>=>{
