...
const PriceCard = ({ plan } : any) => (
  <div className={`flex flex-col overflow-hidden rounded-lg ${plan.featured ? 'border-2 border-indigo-500' : 'border'} ${plan.offset ? 'sm:mt-8' : ''}`}>
    {plan.featured ? (
      <div className="bg-indigo-500 py-2 text-center text-sm font-semibold uppercase tracking-widest text-white">Premium plan</div>
    ) : (
    ...
    <div className="flex flex-1 flex-col p-6 pt-8">
      ...
      <div className="mt-auto">
        <a href="#" className={`block rounded-lg ${plan.buttonColor} px-8 py-3 text-center text-sm font-semibold ${plan.buttonTextColor} outline-none ring-indigo-300 transition duration-100 hover:${plan.buttonHoverColor} focus-visible:ring active:${plan.buttonActiveColor} md:text-base`}>
          {plan.price}
        </a>
      </div>
    </div>
  ...
  return (
    <div className="bg-background py-6 sm:py-8 lg:py-12">
      <div className="mx-auto max-w-screen-xl px-4 md:px-8">
        <h2 className="mb-4 text-center text-2xl font-bold text-primary md:mb-8 lg:text-3xl xl:mb-12">Pricing</h2>
